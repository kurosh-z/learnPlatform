class MSymbols {
  ...
  public static getInstance(): MSymbols {
    if (!MSymbols.instance) {
      MSymbols.instance = new MSymbols();
    }
  ...
  getCharWidth: (
    ...
    if (char in this.widthBank)
      ...
      throw new Error(`the requested char ${char} dose not exist in the Bank`);
  };
  getStringWidth: (
    str: string,
    fontSize?: number,
    ...
    for (const char of chars) {
      strWidth += this.getCharWidth(char, fontSize, fontStyle);
    }
    return strWidth;
  };
}

const mathSymbols = MSymbols.getInstance();
